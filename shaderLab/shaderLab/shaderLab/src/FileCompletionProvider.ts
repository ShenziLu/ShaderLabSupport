...
import { hlslIncludeFiles, tags, Tags } from "./shaderGlobals";

export default class FileCompletionProvider implements CompletionItemProvider{
    ...
    public provideCompletionItems(document: TextDocument, position: Position, token: CancellationToken, context: CompletionContext): Promise<CompletionItem[]> {
        ...
        for (var key in hlslIncludeFiles) {
            if (hlslIncludeFiles.hasOwnProperty(key)) {
                createNewCompletion(CompletionItemKind.Enum,key,hlslIncludeFiles[key]);
            }
        }
